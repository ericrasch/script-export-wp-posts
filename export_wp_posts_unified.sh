...
echo -e "\n${YELLOW}Merging posts data...${NC}"

# Initialize temp file (header will be added by merge process)
> "$TEMP_FILE"

# For now, always use the improved AWK method since Python detection seems unreliable
MERGE_METHOD="AWK"
echo "Using improved CSV parser..."

# Create a Python script for merging
if [ "$MERGE_METHOD" = "Python" ]; then
    cat > "$EXPORT_DIR/merge_csv.py" << 'PYTHON_SCRIPT'
#!/usr/bin/env python3
import csv
import sys

if len(sys.argv) != 4:
    print("Usage: merge_csv.py <permalinks_file> <posts_file> <output_file>", file=sys.stderr)
    sys.exit(1)

permalinks_file = sys.argv[1]
posts_file = sys.argv[2]
output_file = sys.argv[3]

# Read custom permalinks into dictionary
permalinks = {}
try:
    with open(permalinks_file, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if 'ID' in row and 'custom_permalink' in row:
                permalinks[row['ID']] = row['custom_permalink']
except Exception as e:
    print(f"Error reading permalinks: {e}", file=sys.stderr)

# Write output
with open(output_file, 'w', encoding='utf-8') as out:
    # Write header
    out.write("ID,post_title,post_name,custom_permalink,post_date,post_status,post_type\n")
    ...
    try:
        with open(posts_file, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                post_id = row.get('ID', '')
                title = row.get('post_title', '').replace(',', '')  # Remove commas
                post_name = row.get('post_name', '')
                post_date = row.get('post_date', '')
                post_status = row.get('post_status', '')
                post_type = row.get('post_type', '')
                custom_permalink = permalinks.get(post_id, '')
                
                # Write CSV line
                out.write(f"{post_id},{title},{post_name},{custom_permalink},{post_date},{post_status},{post_type}\n")
                
                print(f"Processed row: {post_id}", file=sys.stderr)
    except Exception as e:
...
PYTHON_SCRIPT
    ...
    if $PYTHON_CMD_MERGE "$EXPORT_DIR/merge_csv.py" "$CUSTOM_PERMALINKS_FILE" "$ALL_POSTS_FILE" "$TEMP_FILE"; then
        echo "✓ CSV merge completed successfully"
        rm -f "$EXPORT_DIR/merge_csv.py"
    else
        echo "Python merge failed, falling back to AWK" >&2
        rm -f "$EXPORT_DIR/merge_csv.py"
        MERGE_METHOD="AWK"
        > "$TEMP_FILE"
    fi
...
if [ "$MERGE_METHOD" = "AWK" ]; then
    ...
    # Use perl to properly parse CSV and merge files
    perl -e '
use strict;
use warnings;
